...
// Local secondary price for the active country. For Paraguay this is
// "Gs 550.000.000" (dot separators) — byte-identical to before; we use "Gs" not
// the ₲ glyph because the app fonts (Space Grotesk / IBM Plex Mono) don't include
// ₲, so it would render as a broken box on device. Other countries use their own
// prefix/locale from the country profile. Named `pyg` for call-site compat.
export function pyg(usd) {
  const c = getCountry();
  const v = Math.round((Number(usd) || 0) * c.usdRate);
  return c.moneyPrefix + v.toLocaleString(c.moneyLocale);
}
...
export function toUsd(price, currency) {
  ...
  if (currency === 'PYG' || currency === 'GS' || currency === '₲') return p / getUsdRate();
  return p;
}
